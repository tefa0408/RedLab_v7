...
export const renderImage = (imageUrl) => {
  const backendURL = process.env.REACT_APP_API_IP_BACKEND;
  const backendPORT = process.env.REACT_APP_API_PORT_BACKEND;
  try {
    // To enable backward compactibility images in the backend are stored in
    // host:port/pathname
    ...
    // incase the imageURL entered is just a pathname
    // use the name directly
    return `${backendURL}:${backendPORT}${imageUrl}`;
  }
};
...
export function getRoleFunctionActions(
  menuOptions,
  actions,
  auxFunction,
  criteria = "id"
) {
  if (!_.isEmpty(actions, true)) return actions;
  else {
    let currentFunction = menuOptions.find(function(x) {
      return x[`${criteria}`] === auxFunction;
    });

    if (currentFunction) return currentFunction.roles[0].actions;
    else return [];
  }
}
