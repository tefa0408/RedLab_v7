...
import DialogEditExamination from "./DialogEditExamination";
import DialogEditExamReferences from "./DialogEditExamReferences";
import {
  saveServiceApi,
  saveMethodApi,
  saveUnitApi,
...
class Examination extends Component {
  state = {
    ...
    dialogUnit: false,
    examValue: [],
    examValueDialog: false,
    examReferenceValueDialog: false
  };

  
  openEditExamValues = (examValue) => {
  ...
  closeExamReferenceValueDialog = (event) => {
    this.setState({examReferenceValueDialog: false});
  }
  closeEditExamValues = (event) => {
 
    this.setState({examValueDialog: false});
  }
  ...
  reloadValues = (examinationId) => {
    const { showMessage, fetch_end, fetch_start } = this.props;
    fetch_start();
    getReferenceValueApi(examinationId)
      .then(
        (response) => {
          if (response.status) {
            const index = this.state.examValues.findIndex((exm) => exm.id === examinationId);
            const allExams = this.state.examValues;
            allExams[index] = response.data;
            this.setState({examValues: allExams})
  ...
  reloadExaminationReferences = (currentPageId, currentValueId) => {
    const index = this.state.newReferenceValues.findIndex((exm) => exm.id === currentValueId);
    console.log('aqui pues');
    console.log(index);
    console.log(this.state.newReferenceValues[index]);
    // console.log(index);
    // console.log(this.state.newReferenceValues[index]);
    this.setState({newReferenceValues: []});
    this.fetchExaminationValues(currentPageId, index, currentValueId);
  }

  fetchExaminationValues = (examinationId, indexToSplice, idToSplice) => {
    const { showMessage, fetch_end, fetch_start } = this.props;
    fetch_start();
      ...
      .then(
        (response) => {
          if (response.status) {
            ...
            if(indexToSplice && idToSplice){
              console.log('es mayor o igual así que procede');
              const item = this.state.newReferenceValues.findIndex((exm) => exm.id === idToSplice);
              this.state.newReferenceValues.splice(indexToSplice, 0, this.state.newReferenceValues[item])
              let uniqueItems = [...new Set(this.state.newReferenceValues)];
              this.state.newReferenceValues = uniqueItems;
            }

            if(indexToSplice === 0){
              const index = this.state.newReferenceValues.findIndex((exm) => exm.id === idToSplice);
              this.state.newReferenceValues.unshift(this.state.newReferenceValues.splice(index, 1)[0]);
              console.log('HACIENDOLO ACÁ:' + indexToSplice);
            }
  ...
  addExamValue = async () => {
    ...
    if (examinationId !== "new") {
      d = await editExamination(
        {
          action: "create",
          relationId: selectedGroup.id,
          modelName: "examValue",
        },
        data,
        examinationId
      );
      data.id = d ? d.id : FuseUtils.generateGUID();
    }
  ...
  render() {
    ...
    return (
      <div>
                                ...
                                <TableCell component="th" scope="row">
                                  <Tooltip title="Editar">
                                    <Button
                                      ...
                                      color="primary"
                                      onClick={() =>
                                        this.openEditExamValues(examValue)
                                      }
                                    >
                                      Editar
                                    </Button>
                              ...
                              >
                                ...
                                <Tooltip title="Editar">
                                    <Button
                                      ...
                                      color="primary"
                                      onClick={() =>
                                        this.openEditExamReferencesValue(value)
                                      }
                                    >
                                      Editar
                                    </Button>
        ...
        {this.state.examValueDialog && (
          <DialogEditExamination
              dialog= {this.state.examValueDialog}
              onEdit= {this.closeDialogEdit}
              refreshUpdated={this.refreshUpdated}
              onResponse={this.closeEditExamValues}
              onClose = {this.closeEditExamValues}
              title = "Editar examen"
              examId= {this.state.examValue.id}
              examContent={this.state.examValue}
              examGroups = {this.state.examGroups}
              onAddValue = {this.editValueExam}
              reloadValues={this.reloadValues}
              />
        )}

          {this.state.examReferenceValueDialog && (
          <DialogEditExamReferences
              dialog= {this.state.examReferenceValueDialog}
              onResponse={this.closeEditExamValues}
              onClose = {this.closeExamReferenceValueDialog}
              value= {this.state.examReferenceValue}
              examId = {this.state.examReferenceValue.examValue.id}
              reloadExaminationReferences= {this.reloadExaminationReferences}
