...
class Patient extends Component {
  ...
  handleChangeRegion = (value) => {
    ...
    if (value) this.fetchProvinces(value.id);
  };
  ...
  handleChangeProvince = (value) => {
    ...
    if (value) this.fetchDistricts(value.id);
  };
  ...
  handleBirthChanging = (event) => {
    ...
    let formCopy = { ...this.state.form };
    formCopy.birthDate = event.target.value;
    formCopy.edad = currentAge;
    this.setState({form: formCopy});
  };

  handleAgeChanging = (event) => {
    if(this.state.form.birthDate == null || this.state.form.birthDate == ''){
      let formCopy = { ...this.state.form };
      formCopy.edad = event.target.value;
      this.setState({form: formCopy})
    }

    console.log('BirthDate tiene como valor:' + this.state.form.birthDate);
  ...
  //   if (patientId === "new") this.props.newPatient();
  //   else if (!Object(patient).length)
  //     //If patient is empty, it doesn't come from list, so get from API
  ...
  componentDidMount() {
    ...
    this.setState({form: patient}) // ! rough
    ...
    getRolesApi()
      .then(
        (response) => {
          if (response.status)
            this.setState({
              roles: response.data.filter((r) => r.name == "Rol Paciente"),
            });
        },
        (err) => {
          console.log(err);
  ...
  fetchTypeDocs = () => {
    ...
    getTypeDocsApi()
      .then(
        (response) => {
          if (response.status) this.setState({ typeDocs: response.data });
        },
        (err) => {
          console.log(err);
          showMessage({
  ...
  fetchRegions = () => {
    ...
    getRegionsApi()
      .then(
        (response) => {
          if (response.status) this.setState({ regions: response.data });
        },
        (err) => {
          console.log(err);
          showMessage({
  ...
  fetchProvinces = (regionId) => {
    ...
    getProvincesForRegion(regionId)
      .then(
        (response) => {
          if (response.status) this.setState({ provinces: response.data });
        },
        (err) => {
          console.log(err);
          showMessage({
  ...
  fetchDistricts = (provinceId) => {
    ...
    getDistrictsForProvince(provinceId)
      .then(
        (response) => {
          if (response.status) this.setState({ districts: response.data });
        },
        (err) => {
          console.log(err);
          showMessage({
  ...
  //     !hasEmptyField(form, ["tlfNumber", "username"])
  //   );
  // };
  ...
  submit = () => {
    ...
    //Genera un correo eléctronico aleatorio, usando el año, mes, día, hora, minuto y segundos del momento.
    if(formValue.username == '' || !formValue.username){
      var today = new Date();
      var date = today.getFullYear()+'_'+(today.getMonth()+1)+'_'+today.getDate() + '_';
      var time = today.getHours() + "_" + today.getMinutes() + "_" + today.getSeconds();
      var email = date + time + '@correo.com';

      formValue.username = email;
    }

    //Genera un número de documento acorde a la hora, minutos y segundos del momento con la palabra PASS al principio.

    if (formValue.dni == '') {
      var dniToday = new Date();
      //var dniDate = dniToday.getFullYear()+'_'+(dniToday.getMonth()+1)+'_'+dniToday.getDate() + '_';
      var dniTime = dniToday.getHours() + "_" + dniToday.getMinutes() + "_" + dniToday.getSeconds();
      var newPass = 'pass' + dniTime;

      formValue.dni = newPass;
    }

    // Genera un historyNumber aleatorio para llenar en caso de que tenga el valor NULL
    if(formValue.historyNumber == ''){
      var max = 9999;
      var min = 1;

      var newHistoryNumber = Math.floor(Math.random() * (max - min)) + min;
      formValue.historyNumber = 'NONE-' + newHistoryNumber;
    }
    
    //Setea una fecha de nacimiento estatica en caso de que ésta sea NULL en el formulario.
    if(formValue.birthDate == ''){
      formValue.birthDate = '1999-01-01';
    }
   
    if(formValue.DistrictId == '' || !formValue.DistrictId){
      formValue.DistrictId = null;
    }else {
      formValue.DistrictId = formValue.DistrictId.id;
    } //because backend only needs ID
    ...
    } else editPatient(formValue, formValue.userId);
  };
  ...
  stylesAutoSelects = (name) => ({
    ...
    control: (provided, state) => ({
      ...provided,
      height: "56px",
      borderColor: this.state.form[name]
  ...
  render() {
    ...
    const {
      params: { patientId, patientHandle = "" },
    } = this.props.match;
    const returnUrl =
      patientHandle === "continue"
        ? "/apps/appointments/new/continue"
        : "/apps/patients";
    return (
      <div>
        <FusePageCarded
                  ...
                  <div>
                    ...
                    <div style={{ display: "flex", justifyContent: "center" }}>
                      <TextField
                        ...
                        //required
                        onChange={this.handleChange}
                        helperText="Seleccione un género"
                        margin="normal"
                        variant="outlined"
                        fullWidth
                      >
                    ...
                    <div style={{ display: "flex", justifyContent: "center" }}>
                      <TextField
                        ...
                        //required
                        onChange={this.handleBirthChanging}
                        InputLabelProps={{
                          shrink: true,
                        }}
                        variant="outlined"
                      ...
                      <TextField
                        ...
                        value={form.edad || ''}
                        name="edad"
                        onChange={this.handleAgeChanging}  
                        variant="outlined"
                        className="mt-8 mb-16 mr-8 ml-8"
                        autoComplete="off"
                        InputProps={{
