...
import { reportExamMonthly, reportExamByDate } from '../../../api';
...
class ReportExamMonthlyHeader extends Component {
    ...
    handleChange = (event) => {
        ...
        if(event.target.name == "day"){
          ...
          console.log('State day value:' + this.state.day);
        }else{
          this.setState({
    ...
    fetchReport = () => {
        ...
        if(this.state.day == null){
          ...
          //console.log('DAY TIENE ALGUN VALOR SETEADO');
          
          const {day, month, year, AgreementId, HeadquarterId} = this.state
          const { showMessage, fetch_end, fetch_start, onFetchReport, onFailedReport } = this.props
          fetch_start()
          reportExamByDate(day, month, year, AgreementId, HeadquarterId).then(response => {
              if (response.status){
                  onFetchReport(response.data, day, month, year)
                  this.setState({isLoaded: true})
              }
              else{
                  showMessage({ message: response.message.text, variant: "error" })
