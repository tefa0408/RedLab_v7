import { getAppointmentsApi, getFilterAppointmentsApi, getAppointmentsByReferer, getAppointmentsByDates,getAppointmentsByPersonId } from '../../../../../api';
...
export function searchAppointmentsByReferer(criteria, query, status)
{
    return (dispatch) =>{
        dispatch(fetch_start())
        getAppointmentsByReferer(criteria,query, status).then(response => {
            if (response.status){
                dispatch({
                    type: GET_APPOINTMENTS,
                    payload: response
...
export function searchAppointmentsByDates(start, end, status)
{
    return (dispatch) =>{
        dispatch(fetch_start())
        getAppointmentsByDates(start,end,status).then(response => {
            if (response.status){
...
export function searchAppointmentsByName(personId, status)
{
    return (dispatch) =>{
        dispatch(fetch_start())
        getAppointmentsByPersonId(personId, status).then(response => {
            if (response.status){
                dispatch({
                    type: GET_APPOINTMENTS,
                    payload: response
