...
async function getExaminationValues(req, res){
  try {
    const examinationValues = await sReferenceValue.getAllExaminationValues();
    makeResponseOk(res, { data: examinationValues }, "examinationValue/all");
  } catch (err) {
    makeResponseException(res, err);
  }
}
...
async function updateExaminationReferenceValue(req, res) {
  try {
    let body = req.fields;
    let id = req.params.id;
    await sReferenceValue.updateExaminationReferenceValue(id, body);
    makeResponseOkMessage(res, "I029");
  } catch (err) {
    makeResponseException(res, err);
  }
